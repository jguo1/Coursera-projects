# http://www.codeskulptor.org/iipp-practice-experimental/#user40_NRJ1kgeudz_1.py
...
def new_game():
    ...
    for i in range(len(my_tiles)):
        my_tiles[i].update_position(TILE_NUM_PER_ROW, i)
    state = 0
    turns = 0
...
class Tile:
    ...
    def draw_tile(self, canvas):
        ...
        if self.exposed:
            if self.number < 10:
                ...
                text_location = [loc[0] , loc[1] - 0.3 * TILE_HEIGHT]
    ...
    def update_position(self,row_num,idx):
        self.location[1] = (idx / row_num + 1) * TILE_HEIGHT
        self.location[0] = self.location[0] - (idx / row_num) * row_num * TILE_WIDTH
...
if DISTINCT_TILES * 2 % TILE_NUM_PER_ROW == 0:
    frame = simplegui.create_frame("Memory", TILE_NUM_PER_ROW * TILE_WIDTH, (DISTINCT_TILES * 2 /TILE_NUM_PER_ROW) * TILE_HEIGHT) 
else:
    frame = simplegui.create_frame("Memory", TILE_NUM_PER_ROW * TILE_WIDTH, (DISTINCT_TILES * 2 /TILE_NUM_PER_ROW + 1) * TILE_HEIGHT)
